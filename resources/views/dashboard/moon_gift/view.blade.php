            ...
            <div class="box">
                ...
                <div>
                    <ul class="list-group container">
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Total app profit
                            <span class="badge bg-primary rounded-pill">{{ $diamond_lost - $diamond_win }}</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center">
                            Total app won
                ...
                <div class="p-5">
                    <div class="form-group d-flex flex-row">
                        <form action="{{ route('dashboard.moon-gift.view') }}" method="GET">
                            @csrf
                        ...
                        <a class="btn btn-danger ml-3" href="{{ route('dashboard.moon-gift.reset') }}">Reset</a> 
                    </div>
                </div>
                <table class="table">
                    <thead>
                    ...
                    <tbody>
                        @foreach($data as $key => $items)
                            <tr>
                                <td>{{ $items->sum('diamond_lost') - $items->sum('diamond_win') }}</td>
                                <td>{{ $items->sum('diamond_lost') }}</td>
                                <td>{{ $items->sum('diamond_win') }}</td>
                                <td>{{ $items->count() }}</td>
